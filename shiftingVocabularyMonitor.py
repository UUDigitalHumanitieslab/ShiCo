...
def expandRelatedWords(oModel, aSeedSet, iMaxNrOfTerms=None, fMinDist=None):
  ...
  return [(sWord, fWeight) for sWord, fWeight in dRelatedWords.items()]
...
def trackCloud1(oModel, aTerms, iMaxNrOfTerms=10, iMaxNrOfRelatedTerms=10,
  ...
  while npaEmbeddings.shape[0] > iMaxNrOfTerms:
    ...
    if sDist == 'cosine':
      ...
      print("%s" % list(zip(aTerms, [x[0] for x in npaEuclideanDistances])))
    ...
    npaEmbeddings = npaEmbeddings[list(range(iDelIndex)) + \
                                    list(range(iDelIndex+1, npaEmbeddings.shape[0]))]
    ...
    if iDelIndex2 is not None:
      ...
      # Cut it from the embeddings
      npaEmbeddings = \
          npaEmbeddings[list(range(iDelIndex2)) + \
                          list(range(iDelIndex2+1, npaEmbeddings.shape[0]))]
      # Also cut it from the term list
...
def surroundingWords(oModel, npaWordVector, fMinDist, iTopN, bDebug=False):
  ...
  aSortedKeys = sorted(list(dResults.keys()), key=dResults.__getitem__, reverse=True)
